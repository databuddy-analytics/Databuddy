...
- **Insights:** Set `AI_GATEWAY_API_KEY` in `.env`, then rerun `docker compose -f docker-compose.selfhost.yml up -d --build`. Website research also needs `CONTEXT_DEV_API_KEY`.
