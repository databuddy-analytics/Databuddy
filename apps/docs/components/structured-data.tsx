...
import type { RawPlan, RawItem } from '@/app/(home)/pricing/data';
...
function planToOffer(plan: RawPlan, baseUrl: string) {
  ...
  const basePrice = priceItem && typeof priceItem.price === 'number' ? priceItem.price : 0;

  // Included features → additionalProperty
  const included = plan.items
    .filter((i): i is Extract<RawItem, { type: 'feature' }> => i.type === 'feature')
    .map((i) => ({
      '@type': 'PropertyValue',
      name: i.feature?.name,
      value: i.included_usage === 'inf' ? 'Unlimited' : String(i.included_usage),
  ...
  plan.items
    .filter((i): i is Extract<RawItem, { type: 'priced_feature' }> => i.type === 'priced_feature')
    .forEach((pf) => {
    ...
    if (el.type === 'article') {
      ...
      graph.push({
        ...
        offers: {
          ...
          lowPrice: Math.min(...offers.map((o) => Number(o.price ?? 0))).toFixed(2),
          highPrice: Math.max(...offers.map((o) => Number(o.price ?? 0))).toFixed(2),
          priceCurrency: 'USD',
          offers,
        },
      });
