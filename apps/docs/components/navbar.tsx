						...
						<div className="relative flex items-center h-16">
								...
								<NavigationMenu>
									<NavigationMenuList className="gap-1">
										{navMenu.map((menu) => (
											...
											<Link
												href={menu.path}
												className="inline-flex h-9 w-max items-center justify-center bg-transparent px-4 py-2 text-sm font-medium hover:text-primary disabled:pointer-events-none disabled:opacity-50 transition-all duration-200 ease-out relative after:absolute after:bottom-1 after:left-4 after:right-4 after:h-0.5 after:bg-primary after:origin-left after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-200 after:ease-out focus:outline-none"
											>
												{menu.name}
											</Link>
											</NavigationMenuItem>
							...
							<div className="ml-auto flex gap-5">
								<a
									...
									className={cn(
										'group inline-flex h-8 items-center gap-2 rounded-lg bg-muted/70 hover:bg-muted/90 border border-border/70 hover:border-border/90 px-2 text-sm transition-all duration-200 min-w-[120px] justify-center',
									)}
								>
			...
			<div className="lg:hidden flex flex-col fixed top-0 left-0 right-0 bg-background/95 backdrop-blur-lg border-b border-border/50 z-50 shadow-sm">
				...
				<div
					className={`md:hidden overflow-hidden transition-all duration-300 ease-out ${isMobileMenuOpen
						? 'max-h-[500px] opacity-100 border-b border-border/50'
						: 'max-h-0 opacity-0'
						}`}
				>
							...
							<div className="space-y-1">
								{navMenu.map((menu, index) => (
									<Link
										key={menu.name}
										href={menu.path}
										className={`block px-0 py-3 text-lg font-medium text-foreground hover:text-primary hover:translate-x-1 transition-all duration-200 ease-out hover:font-semibold ${isMobileMenuOpen
								...
								{socialLinks.map((social, index) => {
									...
									return (
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											className={`p-2 text-muted-foreground hover:text-foreground transition-all duration-200 hover:scale-110 ${isMobileMenuOpen
												? 'translate-y-0 opacity-100'
												: 'translate-y-4 opacity-0'
												}`}
											style={{
												transitionDelay: isMobileMenuOpen ? `${(navMenu.length + index) * 30}ms` : '0ms'
											}}
