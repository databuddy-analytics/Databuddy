							...
							<div className="space-y-1">
								{mobileNavLinks.map((link, index) => (
									<Link
										key={link.name}
										href={link.path}
										className={`block px-0 py-3 text-lg font-medium text-foreground hover:text-muted-foreground transition-all duration-200 ${isMobileMenuOpen
								...
								{socialLinks.map((social, index) => {
									...
									return (
										<a
											...
											style={{
												transitionDelay: isMobileMenuOpen ? `${(mobileNavLinks.length + index) * 30}ms` : '0ms'
											}}
...
const mobileNavLinks = [
	...
	{
		name: "Contact Us",
		path: "mailto:support@databuddy.cc",
		external: true,
	}
];
