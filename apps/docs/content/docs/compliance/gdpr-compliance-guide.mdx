---
title: "GDPR Compliance for Web Analytics: Complete Guide"
...
- Avoids analytics-specific consent friction where configuration and jurisdiction allow
...
**No Personal Data Collection by Default:**
- No raw IP address storage (only a truncated salted hash is retained)
- No cross-site tracking or advertising profiles
- Anonymous aggregation for reporting
- Identity is opt-in: personal data enters the system only if you use `identify()`

**Technical Safeguards:**
- Salted hashing of visitor identifiers where anonymization applies
- A single first-party localStorage ID, no third-party identifiers
- Global Privacy Control and Do Not Track honored by default
- Application-layer encryption support for identity fields
...
- Raw IP addresses
- Names, emails, or other personal identifiers (unless you opt in with `identify()`)
- Cross-site data
...
In the default anonymous configuration, most of these rights have little data to act on. When you identify users, the profiles you create are personal data and these rights apply in full. For any request:
- Clear data processing information is published in our [Data Policy](/data-policy)
- Contact [privacy@databuddy.cc](mailto:privacy@databuddy.cc) and we respond promptly
- Profile data created via `identify()` can be corrected or deleted on request
...
<CodeBlock 
  ...
  code={`<!-- Anonymous, cookieless measurement -->
<script
...
By default, Databuddy collects only anonymous data, which is the strongest position for running without an analytics consent banner. Whether one is required depends on your configuration, what you send, and your jurisdiction. If you enable `identify()` or send personal data in events, treat it like any other personal-data processing.
...
For anonymous default tracking there is usually no personal data to act on. If you identify users, data subject rights apply in full to those profiles: contact [privacy@databuddy.cc](mailto:privacy@databuddy.cc) and see our [Data Policy](/data-policy) for how data flows through the system.
...
- ✅ Start from a GDPR-compliant anonymous default
