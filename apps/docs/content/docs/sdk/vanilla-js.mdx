...
<Callout type="info">
  **CDN**: `https://cdn.databuddy.cc/databuddy.js` | **Current size**: about 40 KB minified / 12 KB gzip
</Callout>
...
The script is automatically minified and served compressed. The current core bundle is about 40 KB minified and 12 KB gzip; this includes the error and web vitals trackers, which stay inactive unless `data-track-errors` or `data-track-web-vitals` are enabled. Standalone `errors.js` (about 21 KB) and `vitals.js` (about 26 KB) builds are also published on the CDN. Check release notes for future size changes.
