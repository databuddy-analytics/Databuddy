...
Create an API key with the `track:events` scope in your organization settings, then set two environment variables:
...
`DATABUDDY_WEBSITE_ID` also works in place of `NEXT_PUBLIC_DATABUDDY_CLIENT_ID`. If either variable is missing, nothing is sent.
...
export default {
  async fetch(request, env, ctx) {
    ctx.waitUntil(
      trackAgents(request, { apiKey: env.DATABUDDY_API_KEY, websiteId: "your_website_id" })
    );
...
Workers don't have `process.env`, so pass the key and website id as options.
