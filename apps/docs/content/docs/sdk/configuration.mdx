...
<Callout type="info">
  Use `trackWebVitals` in React, Vue, and Nuxt, or `data-track-web-vitals` with the CDN script.
</Callout>
...
The browser SDK auto-detects the client ID from `NEXT_PUBLIC_DATABUDDY_CLIENT_ID`, `NUXT_PUBLIC_DATABUDDY_CLIENT_ID`, `VITE_DATABUDDY_CLIENT_ID`, or `REACT_APP_DATABUDDY_CLIENT_ID`. Other variables must be wired in explicitly.
...
The Node client does not read environment variables on its own. Pass them through the constructor:
...
const client = new Databuddy({
  apiKey: process.env.DATABUDDY_API_KEY!,
  websiteId: process.env.DATABUDDY_WEBSITE_ID,
  apiUrl: process.env.DATABUDDY_API_URL
});`}
</CodeBlock>
