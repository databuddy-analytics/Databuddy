...
<CodeBlock language="tsx">
  {`interface DatabuddyConfig {
    ...
    requestTimeoutMs?: number;    // Default: 10000ms request deadline
...
<CodeBlock language="tsx">
  ...
  code?: string;         // e.g. "NETWORK_ERROR", "VALIDATION_ERROR", "QUEUE_FULL"
  why?: string;
  fix?: string;
  requestId?: string;
  retryable?: boolean;
...
<Callout type="warn">
Server-side events are stored as custom events. They do not create the page-view
sessions that referrer, campaign, country, and device reporting is built on, and
[revenue attribution](/docs/Integrations/payments) cannot join to them. Passing a
`sessionId` you generated on the server does not change this. To attribute
server-side activity, take the real `sessionId` from `getTrackingIds()` in the
browser and send it to your server.
</Callout>
...
Link your user's stable ID to their tracked activity and set profile traits:

<CodeBlock language="tsx">
  {`await client.identify({
  profileId: 'user-123',
  anonymousId: 'anon_...',  // Optional: link pre-login device activity
  traits: {
...
`profileId` is required. `websiteId` comes from the config default or can be passed per call. Pass the client's raw `anonymousId` (from the web SDK's `getAnonymousId()`) to connect pre-login events from that device. Requires an API key with the `track:events` scope for the target website. Identify calls send immediately and are not batched. See [Identify Users](/docs/sdk/identify-users) for concepts and trait rules.
...
<Callout type="info">
  The Node SDK imports `node:crypto`, so Cloudflare Workers require the `nodejs_compat` compatibility flag in your `wrangler.toml`.
</Callout>
