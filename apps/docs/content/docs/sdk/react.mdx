...
|------|---------|-------------|
| `trackWebVitals` | `false` | FCP, LCP, INP, CLS, TTFB, and FPS |
...
The identity helpers (`identify`, `setTraits`, `clearProfile`, `getProfileId`) and the flags hooks (`FlagsProvider`, `useFlag`, `useFlags`) are also exported from `@databuddy/sdk/react`.
