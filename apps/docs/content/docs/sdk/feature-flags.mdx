...
<Callout type="info">
  **React vs Vue API differences**: In Vue, `useFlag` returns `{ on, loading, state }` as refs; there are no top-level `status`, `value`, or `variant` fields, so read them from `state` (e.g. `flag.state.value.variant`). Vue's `useFlags` returns `{ getFlag, fetchAllFlags, updateUser, refresh, updateConfig, memoryFlags, lastError }`; it has no `isOn`, `getValue`, `fetchFlag`, or `isReady`. The `getValue`, `isOn`, and `variant` examples below are React-only.
</Callout>
...
<Callout type="info">
  **Variant weights**: If no variant weights are configured, users are split evenly. If you add weights, they must total exactly 100; the dashboard rejects flags whose weights do not sum to 100.
</Callout>
...
### Visibility API
Pauses background revalidation when the tab is hidden to save bandwidth and battery. Initial fetches still run.
