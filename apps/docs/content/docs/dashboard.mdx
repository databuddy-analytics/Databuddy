...
- **Today** and **Yesterday** - Real-time insights
- **Last 7 days** and **Last 14 days** - Weekly trends
- **Last 30 days**, **This month**, and **Last month** - Monthly overview
...
<Callout type="info">
  **Comparison Mode**: Your selected period is automatically compared with the
...
<Callout type="info">
  **Privacy Note**: Location is derived from the visitor's IP address at
  ingestion and stored at city level. The IP address itself is not stored, and
...
<Callout type="tip" title="Enabling Error Data Collection">
  Error tracking requires the Hobby plan or higher. To populate the error
  tracking sections of the dashboard, ensure you have `trackErrors={true}`
  enabled in your [SDK configuration](/docs/sdk/configuration). This will
  automatically capture unhandled JavaScript errors and promise rejections. For
  more detailed context on specific handled errors, you can manually send error
  events using `db.track('error', {/* custom properties */})` as described in
  the [Tracker Helpers](/docs/sdk/tracker) guide.
</Callout>
...
- **Unhandled JavaScript errors** - Runtime exceptions with stack, page, and session context
- **Unhandled promise rejections** - Async failures with the same context
...
<Cards>
  <Card title="Data Export">
    Download your data as JSON or CSV via Website Settings > Data Export.
  </Card>
...
<Callout type="info">
  Uptime monitor alerts can be delivered to a webhook destination, but there is
  no outbound webhook streaming for analytics data.
</Callout>
