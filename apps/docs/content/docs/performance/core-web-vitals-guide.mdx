...
Databuddy already stores every vital per route, so there is nothing extra to track. Compare the Vitals dashboard against Google Search Console for the same period: routes that moved from Poor to Good LCP or INP are the ones most likely to see ranking and organic traffic recover.
