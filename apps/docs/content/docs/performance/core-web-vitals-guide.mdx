...
<CodeBlock 
  language="tsx"
  code={`// Enable Web Vitals once; LCP is captured automatically from real users
<Databuddy
...
LCP then appears in your dashboard's Vitals tab, per route, with p50 to p99 percentiles.
...
With `trackWebVitals` enabled, INP is captured automatically using Google's web-vitals library and re-measured on SPA route changes. No extra code is needed; see per-route INP in the Vitals dashboard.
...
With `trackWebVitals` enabled, CLS is captured automatically for the full page lifespan and re-measured on SPA route changes. No extra code is needed; see per-route CLS in the Vitals dashboard.
...
Databuddy already stores every vital per route, so there is nothing extra to track. Compare the Vitals dashboard against Google Search Console for the same period: routes that moved from Poor to Good LCP or INP are the ones most likely to see rankings and organic traffic recover.
...
- **About 12 KB gzipped** - Current tracker bundle size
...
The Vitals dashboard compares every metric against the previous period automatically, so before/after reporting needs no custom events: ship the fix, pick the date range that spans it, and the per-route trend shows whether the improvement held.
