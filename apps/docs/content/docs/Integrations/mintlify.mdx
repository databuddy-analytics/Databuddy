...
<Callout type="info">
  There is no official Databuddy integration for Mintlify *yet*. The setup below is the recommended manual approach used by docs sites today, including [Notra](https://github.com/usenotra/notra/blob/main/apps/docs/databuddy.js).
</Callout>
...
## Alternative: customScripts in docs.json

If you prefer to be explicit about which scripts ship with your docs, or you want to keep `databuddy.js` outside the auto-load behavior, add it to the `customScripts` array in `docs.json`.

<Steps>
  <Step title="Place databuddy.js at the Project Root">
    Keep the same `databuddy.js` file from above. Make sure it lives at the root of your Mintlify project (the same directory as `docs.json`).
  </Step>

  <Step title="Reference It in docs.json">
    Add the file path to `settings.customScripts`. The path must start with `/`.
    ...
    <CodeBlock
      ...
      code={`{
  "settings": {
    "customScripts": ["/databuddy.js"]
  }
}`}
    />

    If you already have a `settings` block, just append `"/databuddy.js"` to the existing `customScripts` array.
  </Step>
</Steps>
...
2. If you went the `customScripts` route, double check the path starts with `/`.
3. Make sure your changes are deployed. Mintlify only includes the script on the published site, not the local preview.
4. Check the browser console and Network tab for blocked requests or 404s.
</Callout>
...
Need help with your Mintlify integration? Contact us at [help@databuddy.cc](mailto:help@databuddy.cc).
