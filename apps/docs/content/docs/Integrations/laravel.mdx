...
Mark the form with `data-form-type` so the submit handler from `resources/js/app.js` (shown above) tracks it. Avoid adding `data-track` to the form itself: that attribute fires a click event for every click inside the form and would duplicate the submit handler.

<CodeBlock
  language="php"
  code={`<form method="POST" action="{{ route('contact.store') }}" data-form-type="contact">
    @csrf
    <!-- form fields -->
