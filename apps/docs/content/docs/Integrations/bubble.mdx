...
<Callout type="info">
  **Page View Tracking in Bubble Apps**: Bubble applications often behave like Single Page Applications (SPAs) for navigation between "pages" (which are often groups shown/hidden on a single actual HTML page). Databuddy automatically detects URL changes in SPAs.
  ...
  - If your Bubble app uses actual URL route changes for different views, Databuddy tracks these automatically
  - If navigation primarily involves showing/hiding groups without URL changes, Databuddy will only track the initial page load. Track those "virtual" views as custom events instead: fire `window.databuddy.track('screen_viewed', { screen: 'dashboard' })` from a Bubble workflow action that runs JavaScript when a "page" (group) becomes visible
</Callout>
...
Bubble group swaps do not change the URL, so they cannot be tracked as page views. Track them as custom events instead:

<CodeBlock
  language="javascript"
  code={`// In Bubble's Run JavaScript action (triggered when a group/page becomes visible)
if (typeof window !== 'undefined' && window.databuddy) {
  window.databuddy.track('screen_viewed', {
    screen: 'dashboard'
  });
}`}
/>
...
Databuddy supports automatic tracking via `data-track` attributes, but Bubble does not let you set custom attributes on its native elements. Use workflow-triggered "Run JavaScript" actions calling `window.databuddy.track()` instead, as shown above.
...
- Track these as custom events with `window.databuddy.track('screen_viewed', { screen: '...' })`
