...
- **One payment type never attributes:** your endpoint is probably missing one of the required events. Revenue settings lists each one with when it was last received, so anything showing **Never received** is the place to look. Stripe subscribes an endpoint to a fixed event list, and adding a new payment flow does not update it.
