...
<CodeBlock 
  ...
  code={`// Cookieless approach - a first-party anonymous ID, no cookies
  databuddy.track('page_view', {
    page: window.location.pathname,
    referrer: document.referrer,
    // No cookies and no cross-site identifiers
  });`}
/>
...
- **Chrome**: Restricting third-party cookies, with plans that keep shifting
...
- Avoids analytics-specific consent friction in the default anonymous configuration
- Full feature set (funnels, goals, web vitals, and more)
...
- **Anonymous by default, with identity strictly opt-in**
- **Built for GDPR and CCPA compliance**
- **Respects user privacy preferences, including GPC and DNT**
...
- Revisit whether your consent banner is still needed
- Monitor performance
...
Anonymous cookieless measurement is the strongest position for running without an analytics consent banner. The answer still depends on your configuration and jurisdiction, and changes if you collect personal data such as identified user profiles.

### How does cookieless analytics handle returning visitors?
A first-party anonymous ID stored in the browser recognizes returning visitors without cookies or cross-site tracking.
