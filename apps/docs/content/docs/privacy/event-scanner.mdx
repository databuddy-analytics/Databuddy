...
- `npx @databuddy/scan <folder>` sends no code from outside that folder. The only file it reads outside the folder is the repository root's `.env`, and only to look for `AI_GATEWAY_API_KEY`.
