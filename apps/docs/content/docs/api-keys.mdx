...
- Scopes: `read:data`, `track:events`, `track:llm`, `read:links`, `write:links`, `manage:websites`, `manage:flags`, `manage:config`
...
- **track:llm**: LLM analytics and AI observability tracking
...
- **manage:config**: Revenue config, integrations, organization settings
