...
- **No Cross-Site Fingerprinting** - Device data is used for on-site analytics only. Global Privacy Control and Do Not Track signals are honored by default
- **No Raw IP Addresses** - Used for location (country/region); after that, only a truncated salted hash is retained
...
If you use `identify()` to attach names, emails, or traits to profiles, those profiles are personal data you control: reflect them in your privacy policy and consent flow. Whether a consent banner is required depends on your configuration, what you send, and your jurisdiction's rules; when in doubt, ask your counsel.
...
This changes if you enable `identify()` or send personal data in event properties: your normal consent and privacy-policy obligations then apply.
