...
**Transport notes**: The endpoint accepts POST only; other HTTP methods return `405`. Per-tool rate limits apply: `get_data` 30/min, `reply_to_investigation` 20/min, most other tools 60/min.
...
| `reply_to_investigation` | Ask an included clarification using the saved investigation evidence. |
| `list_insights` | List published findings, including quiet ones. |

`reply_to_investigation` returns the durable reply status immediately. If it is `queued` or `running`, call `get_investigation` with the same investigation ID until the reply is `succeeded` or `failed`; the clarification answer appears in that timeline. This does not fetch fresh data or change the investigation’s action. Start a new question or fresh analysis in the dashboard, where the $1 price is shown before you submit.
