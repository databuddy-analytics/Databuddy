...
<CodeBlock 
  ...
  code={`{
  "success": true,
  "types": ["summary_metrics", "top_pages", "traffic_sources", "browser_name", "..."],
  "configs": {
    "summary_metrics": {
...
<CodeBlock 
  language="json"
  code={`{
  "parameters": ["summary_metrics", "top_pages", "browser_name"],
...
| `granularity` | string | No | `"hourly"` or `"daily"`, aliases `"hour"` and `"day"` (default: daily). Can also be set per parameter |
| `sortBy` | string | No | Field to sort results by |
| `sortOrder` | string | No | `"asc"` or `"desc"` |
| `timeZone` | string | No | Timezone for date calculations |

<Callout type="info">
  Error tracking starts on the Hobby plan. On the free plan, the error query types (`recent_errors`, `error_types`, `errors_by_page`, `error_summary`, `error_chart_data`, `error_trends`, `error_frequency`, `errors_by_type`) return `402` with code `FEATURE_UNAVAILABLE`.
</Callout>
...
| `path` | page and session queries | `{"field": "path", "op": "starts_with", "value": "/blog"}` |
| `referrer` | traffic queries | `{"field": "referrer", "op": "contains", "value": "google"}` |
