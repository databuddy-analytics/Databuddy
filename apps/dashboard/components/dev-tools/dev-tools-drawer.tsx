		...
		<div>
			...
			<RadioGroup className="gap-2" onValueChange={setValue} value={value}>
				{setting.options.map((option) => {
					...
					return (
						<label
							className="flex cursor-pointer items-start gap-3 rounded border p-3 transition-colors hover:bg-accent [&:has([data-state=checked])]:border-primary [&:has([data-state=checked])]:bg-primary/5"
							htmlFor={inputId}
							key={option.value}
						>
							<RadioGroupItem
