 ...
 />
				...
				<div className="relative z-10 flex items-start justify-between">
					<div className="flex items-center gap-2.5">
						{/* EMV Chip */}
						<div className="relative flex h-8 w-10 items-center justify-center overflow-hidden bg-linear-to-br from-amber-200 via-amber-300 to-amber-400 shadow-sm">
							<div className="absolute inset-0.5-[2px] bg-linear-to-br from-amber-100 to-amber-300 opacity-60" />
							<div className="relative grid h-5 w-6 grid-cols-3 grid-rows-3 gap-px">
								{Array.from({ length: 9 }).map((_, i) => (
									<div
										className="h-px bg-amber-600/40"
										key={i.toString()}
									/>
								))}
							</div>
...
function CardBrandLogo({ brand }: { brand: string }) {
 ...
 <div className="size-5 bg-[#eb001b]" />
 <div className="-ml-2.5 size-5 bg-[#f79e1b] mix-blend-hard-light" />
 </div>
 </div>
 );
 }
