...
test("saves activation definitions through oRPC, recovers unfinished edits, and restores history", { tag: "@regression" }, async ({ authenticatedPage: page }) => {
	await page.goto("/organizations/settings/business-context");
	await page.getByRole("button", { name: /^Add definition for/ }).click();
	const outcome = page.getByRole("textbox", { name: "Business outcome", exact: true });
	const activation = page.getByRole("combobox", { name: "Activation event", exact: true });
	const returning = page.getByRole("combobox", { name: "Return event", exact: true });
	...
	const saved = page.waitForResponse((response) => response.url().endsWith("/rpc/businessContext/save"));
	...
	await expect(page.getByRole("button", { name: "Return window: 7 days" })).toBeVisible();
	await page.getByRole("button", { name: /^Remove definition for/ }).click();
	await page.getByRole("button", { name: "Save changes", exact: true }).click();
	await expect(page.getByText("Changes saved", { exact: true })).toBeVisible();
	await page.reload();
	await expect(page.getByRole("button", { name: /^Add definition for/ })).toBeVisible();
});
