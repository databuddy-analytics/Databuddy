...
describe("Autumn webhook inbox", () => {
	iit(
		"keeps the first payload and moves one claim through replay states",
		async () => {
			...
			expect(await listReplayableAutumnWebhookIds({ now: replayAt })).toContain(
				id
			);
	...
	iit(
		"allows one claim and keeps both completion orders terminal",
		async () => {
	...
	iit(
		"bounds retry delay and retains alerted dead letters for audit",
		async () => {
			...
			expect(await deleteDeadLetterAutumnWebhooks({ olderThan: cutoff })).toBe(
				1
			);
	...
	iit(
		"deletes only completed rows older than the retention window",
		async () => {
			...
			expect(await deleteCompletedAutumnWebhooks({ olderThan: cutoff })).toBe(
				1
			);
			expect(await getAutumnWebhook(oldId)).toBeNull();
			expect(await getAutumnWebhook(recentId)).not.toBeNull();
