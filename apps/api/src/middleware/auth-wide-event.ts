...
import { applyAuthWideEvent, type ResolvedAuth } from "@/lib/auth-wide-event";
...
export async function enrichRequestAuthWideEvent(
	request: Request,
	applyAuth: (headers: Headers) => Promise<ResolvedAuth> = applyAuthWideEvent
): Promise<ResolvedAuth | Response | undefined> {
	if (!shouldResolveAuthForWideEvent(request)) {
		return;
	}

	try {
		return await applyAuth(request.headers);
	} catch (error) {
