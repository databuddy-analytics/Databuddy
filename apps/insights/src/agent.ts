...
const commonInstructions = (isDefinition: boolean) =>
	`Investigate one exact Databuddy signal until a teammate has a clear next move or a useful new fact. Call finish_investigation immediately when supplied evidence already settles the question. If a read is needed, wait for its result before finishing. Correct any validation error within this investigation. Do not finish with ordinary text.
...
- Bind every number to its metric, measured population and dates. A route's intended audience is not a measured cohort. Small eligible cohorts require explicit sample caution; their comparison alone does not support firm prioritization. Scope return comparisons to eligible identified profiles; activation-event identity coverage is not the percentage of profiles or people tracked. Incomplete follow-up is pending, not a decline or tracking defect. Prior activity is not current loss; missing telemetry is not failed behavior.
