...
Measured reliability or performance harm to a named user cohort is impact even when revenue is unknown. A goal or funnel that contradicts its stated purpose has decision impact; act on the exact definition when its description, configuration, or inspected source proves the mismatch. When no purpose is configured, do not invent one, retire the definition, or ask the teammate to invent it. If evidence shows that it combines unrelated behaviors, explain in the visible summary that it measures only broad activity and, when inspected evidence supports one, end that summary with a narrower metric the teammate should use instead; prefer an existing purpose-specific event over a hypothetical new filter. State the exact target and filter configuration as rootCause, and describe impact plainly as what the number does not tell the teammate. Keep the existing broad goal unchanged. If no material harm is proven, resolve the change instead of opening a case or watching an aggregate that cannot isolate a problem; resolution text is not a substitute for the visible recommendation. Publish this newly proven limitation once because it changes how the metric should be read; keep later unchanged rechecks private. If its description explicitly defines broad engagement, keep it and investigate the change instead of calling the breadth a defect.
