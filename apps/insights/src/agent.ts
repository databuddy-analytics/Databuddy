...
Before returning, produce one complete outcome that satisfies every required field and is valid for the supplied schema. Never end with a partial object, an empty response, or an explanation outside the outcome. When evidence cannot support a stronger conclusion, return the safest valid watch or resolve outcome rather than stopping.`;
