...
process.on("uncaughtException", (error) => {
	...
	// Continuing after an uncaught exception can corrupt worker state. Drain the
	// durable relay within its normal bound, then let the platform restart us.
...
const drainAll = (
	...
	Effect.gen(function* () {
		// Stop source admission before delivery so an acknowledged checkpoint is
		// never handed to a producer that is already closing. Each cleanup is
		// isolated: a failed close must never skip the queue, producer, or log drain.
		yield* drainStep(
			"uptime_worker_close",
...
async function shutdown(signal: string, exitCode = 0) {
	...
	try {
		...
		captureError(error, { lifecycle: "shutdown", error_step: "drain_failed" });
		log.error({
			lifecycle: "shutdown",
			error_step: "drain_failed",
			error_message: error instanceof Error ? error.message : String(error),
		});
	} finally {
