...
const transport = spyOn(globalThis, "fetch").mockImplementation(
	async (input, init) => {
		...
		if (url.hostname !== "api.useautumn.com") {
			...
			return Response.json(
				{
					...
					flag: null,
					balance: {
						feature_id: balanceFeature,
						granted: 100,
						remaining: allowed ? 50 : 0,
						usage: allowed ? 50 : 100,
						unlimited: false,
...
mock.module("@databuddy/rpc/organization", () => ({
	getOrganizationOwnerId: async () => customerId,
}));
...
const { getAgentBillingAccess, trackAgentUsageAndBill } = await import(
	"./execution"
);
...
describe("agent credits at the native Autumn boundary", () => {
	...
	it("denies exhausted credits and rejects a missing billing owner", async () => {
		...
		await expect(getAgentBillingAccess(null)).rejects.toThrow(
	...
	it.each([
		202, 500,
	])("fails closed on native customer status %s", async (status) => {
	...
	it.each([
		202, 500,
	])("fails closed on native credit check status %s despite an allowed body", async (status) => {
		checkStatus = status;
		await expect(getAgentBillingAccess(customerId)).rejects.toThrow();
		expect(requests).toHaveLength(2);
	});
	...
	it("does not use an unrelated balance as credit permission", async () => {
		...
		await expect(getAgentBillingAccess(customerId)).rejects.toThrow(
			"credit balance could not be verified"
		);
	});
	...
	it("rejects pinned permission belonging to another customer", async () => {
		await expect(
			trackAgentUsageAndBill({
				billingCustomerId: customerId,
				billingAccess: { allowed: true, customerId: "another-customer" },
				modelId: "openai/gpt-5.6-luna",
				source: "slack",
				usage,
			})
