...
describe("analytics tool contract", () => {
	it.each([{ target: "event" }, { having: false }])(
		"rejects unsupported filter scope instead of silently stripping it: %o",
		async (scope) => {
			const schema = asSchema(getDataTool.inputSchema);
			if (!schema.validate) throw new Error("Missing tool schema validator");
			const result = await schema.validate({ queries: [{
				type: "custom_events_by_path",
				filters: [{ field: "event_name", op: "eq", value: "activation_completed", ...scope }],
			}] });
	...
	it("exposes the exact continuation selector contract before the model queries", async () => {
		if (!discoverQueryTypesTool.execute) throw new Error("Missing discovery tool");
		const result = await discoverQueryTypesTool.execute({ search: "error_route_continuation_comparison" }, options);
	...
	it("returns the measured scope and distinguishes a truncated result from its query row count", async () => {
		const execute = vi.spyOn(SimpleQueryBuilder.prototype, "execute").mockImplementation(function () {
			const compiled = this.compile();
			expect(compiled.params).toMatchObject({ f0: "activation_completed" });
			expect(compiled.sql).toContain("event_name = {f0:String}");
			return Promise.resolve(Array.from({ length: 25 }, (_, index) => ({ name: `/step-${index}`, total_events: 1 })));
		});
		if (!getDataTool.execute) throw new Error("Missing data tool");
		const result = await getDataTool.execute({ queries: [{
			type: "custom_events_by_path",
			from: "2026-08-29",
			to: "2026-09-04",
			filters: [{ field: "event_name", op: "eq", value: "activation_completed" }],
		}] }, options);
		expect(result).toMatchObject({ results: { custom_events_by_path: {
			websiteId: "site-test", from: "2026-08-29", to: "2026-09-04",
			filters: [{ field: "event_name", op: "eq", value: "activation_completed" }],
			returnedRows: 20, rowCount: 25, truncated: true,
		} } });
