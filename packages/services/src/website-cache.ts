...
type WebsiteCacheRecord = Omit<Website, "createdAt" | "updatedAt" | "deletedAt"> & {
    createdAt: string;
    updatedAt: string;
...
export class WebsiteCache {
    ...
    private getRedisClient(): RedisClient | null {
        ...
        try {
            ...
            this.isHealthyCached = false;
            console.error("WebsiteCache: failed to create Redis client", { error: String(error) });
            return null;
        }
    }
    ...
    pingRedis(force = false): Promise<boolean> {
        const now = Date.now();
        if (!force && this.isHealthyCached !== null && now - this.lastPingAtMs < this.pingIntervalMs) {
        ...
        this.pingInFlight = redis
            ...
            .catch((error) => {
                this.isHealthyCached = false;
                console.error("WebsiteCache.pingRedis failed:", { error: String(error) });
                return false;
            })
            .finally(() => {
    ...
    async getWebsiteById(id: string): Promise<Website | null> {
        ...
        try {
            ...
            this.isHealthyCached = false;
            console.error("WebsiteCache.getWebsiteById failed:", { error: String(error) });
            return null;
        }
    }
    ...
    async setWebsite(website: Website): Promise<void> {
        ...
        try {
            ...
            await redis.set(this.keyWebsiteById(website.id), JSON.stringify(toRecord(website)));
        } catch (error) {
            this.isHealthyCached = false;
            console.error("WebsiteCache.setWebsite failed:", { error: String(error) });
        }
    }
    ...
    async deleteWebsiteById(id: string): Promise<void> {
        ...
        try {
            ...
            this.isHealthyCached = false;
            console.error("WebsiteCache.deleteWebsiteById failed:", { error: String(error) });
        }
    }

    async getWebsiteByDomain(domain: string, scope: WebsiteScope): Promise<Website | null> {
        if (!(await this.shouldUseCache())) {
            return null;
        }
        ...
        try {
            ...
            this.isHealthyCached = false;
            console.error("WebsiteCache.getWebsiteByDomain failed:", { error: String(error) });
            return null;
        }
    }

    async setWebsiteByDomain(domain: string, scope: WebsiteScope, website: Website): Promise<void> {
        if (!(await this.shouldUseCache())) {
            return;
        }
        ...
        try {
            ...
            await redis.set(this.keyByDomain(domain, scope), JSON.stringify(toRecord(website)));
        } catch (error) {
            this.isHealthyCached = false;
            console.error("WebsiteCache.setWebsiteByDomain failed:", { error: String(error) });
        }
    }

    async deleteWebsiteByDomain(domain: string, scope: WebsiteScope): Promise<void> {
        if (!(await this.shouldUseCache())) {
            return;
        }
        ...
        try {
            ...
            this.isHealthyCached = false;
            console.error("WebsiteCache.deleteWebsiteByDomain failed:", { error: String(error) });
        }
    }

    async getList(filter: { userId?: string; organizationId?: string }): Promise<Website[] | null> {
        if (!(await this.shouldUseCache())) {
            return null;
        }
    ...
    async setList(filter: { userId?: string; organizationId?: string }, websites: Website[]): Promise<void> {
        if (!(await this.shouldUseCache())) {
            return;
        }
    ...
    async invalidateLists(scopes: {
        ...
        try {
            ...
            this.isHealthyCached = false;
            console.error("WebsiteCache.invalidateLists failed:", { error: String(error) });
        }
    }
}
