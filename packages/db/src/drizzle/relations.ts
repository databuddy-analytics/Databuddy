...
export const alarmTriggersRelations = relations(alarmTriggers, ({ one }) => ({
	alarm: one(alarms, {
		fields: [alarmTriggers.alarmId],
		references: [alarms.id],
	}),
	website: one(websites, {
		fields: [alarmTriggers.websiteId],
		references: [websites.id],
	}),
}));
