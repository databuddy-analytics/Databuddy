...
describe("status page health", () => {
	...
	it("marks missing, invalid, and overdue checks as uncertain", () => {
		...
		expect(deriveMonitorFreshness("not-a-date", "minute", now)).toBe("unknown");
		expect(deriveMonitorFreshness("2026-07-11 11:50:00", "minute", now)).toBe(
			"stale"
		);
		expect(
			deriveMonitorFreshness(
	...
	it("uses the monitor cadence when deciding freshness", () => {
		expect(deriveMonitorFreshness("2026-07-11 11:56:00", "minute", now)).toBe(
			"fresh"
		);
		expect(deriveMonitorFreshness("2026-07-11 09:30:00", "hour", now)).toBe(
			"fresh"
		);
		expect(deriveMonitorFreshness("2026-07-11 08:30:00", "hour", now)).toBe(
			"stale"
		);
	});
	...
	it("does not report empty or uncertain monitor sets as operational", () => {
		...
		expect(
			deriveOverallStatus([{ currentStatus: "unknown", freshness: "unknown" }])
		).toBe("unknown");
		expect(
			deriveOverallStatus([
