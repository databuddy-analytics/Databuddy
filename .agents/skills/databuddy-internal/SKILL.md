...
- One-off internal telemetry analysis does not authorize a dashboard route, RPC endpoint, report UI, config surface, persistence projection, or backfill; add only the requested instrumentation unless a persistent product surface is explicit.
...
- When quarantining legacy `insight_observations`, filter before pagination for chronological history, but select the physical latest row before applying trust in current-state, action, and recheck readers; otherwise an older clean row can be revived as current work.
...
- After an explicitly requested insight evaluation checkpoint, continue into the agreed next slice unless the user asks to pause; report the evidence without treating it as the end of the work.
- When asked whether insight intelligence improved, lead with frozen-data outcome metrics and state plainly when there is no valid before/after or human-quality baseline; passing tests or a cleaner UI is not evidence of an intelligence lift.
...
- Treat output-quality evaluation as measurement, not a separate deterministic copy helper or hard text gate. When it finds noise, remove duplicate upstream context or improve the agent-owned contract.
...
- Recommendation actions must stay in context: use one typed native-action seam with the existing editor/confirmation surfaces, and retire a saved recommendation from durable/live state rather than routing to a configuration page or relying on component-local completion state.
