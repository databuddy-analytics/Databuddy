...
- Jev candidate selection uses the pinned Gateway evaluation SDK; keep all retained preferences until native portfolio grouping/limits and preserve original investigation questions. Judge changes by useful findings and end-to-end latency, not selector cost alone. Validate the complete answer map, probability bounds and declared rounding before using probabilities to exclude work.
